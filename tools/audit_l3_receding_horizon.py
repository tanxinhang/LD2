#!/usr/bin/env python
"""D0.93 L3-G3: receding-horizon capability-sensitivity geometry repair.

On the real 8/8 hard frames (capability gauge gamma* > 1, or target ceiling
below the worst floor), run a short receding horizon of "compute pi*-weighted
geometry gradient -> trust-region move -> Friis rescale the gain -> re-solve the
capability gauge".  The Friis rescale is valid only inside the DD-support-stable
trust region, so each step is capped by a physical radius.

Reports how many hard states reach gamma* <= 1 and the mean capability decrease.
"""
...
def _solve(gain, budget, p_fa):
    """Solve the PWL capability gauge, returning (gamma, power, prices)."""
    ...
    return capability_gauge_pwl_lp_full(
...
def _friis_rescale(gain, owner, uav, tgt, new_uav):
    """Rescale the fixed-owner gain by the Friis distance ratio (DD stable)."""
    K, Q = gain.shape
    ...
    # C_iq = a_iq * R_iq^2 * R_owner^2 ; a'_iq = C_iq / (R'^2 R'^2).
    c = gain * (rtx ** 2) * (rrx[None, :] ** 2)
    new_gain = c / (rtx_n ** 2 * rrx_n[None, :] ** 2)
    return new_gain
...
def run(trace_path, config_path, *, horizon, step_m, seed_limit, max_frames):
    ...
    # Collect hard frames (gamma > 1 or unsolvable).
    hard = []
    for seed in order:
    ...
    for row, gain, owner, uav, tgt, budget in hard:
        ...
        for _ in range(horizon):
            ...
            if out is None:
                break
            gamma_cur, p, pi = out
            g = capability_geometry_gradient(gain, owner, uav, tgt, p, pi)
            # Trust-region step along -gradient, capped.
            delta = -step_m * g
            n = np.linalg.norm(delta, axis=1)
            if np.any(n > step_m):
                delta = delta / np.maximum(n / step_m, 1e-9)[:, None]
            new_uav = uav + delta
            new_gain = _friis_rescale(gain, owner, uav, tgt, new_uav)
            out2 = _solve(new_gain, budget, p_fa)
            if out2 is None:
                break
            if out2[0] >= gamma_cur - 1e-9:
                break  # no strict decrease: stop (Armijo fails)
            gain, uav, gamma = new_gain, new_uav, out2[0]
        final_gammas.append(gamma)
        decreases.append(gamma0 - gamma)
        if gamma <= 1.0:
            reached += 1
    ...
    return {
        ...
        "mean_final_gamma": float(np.mean(final_gammas)) if final_gammas else float("nan"),
        "horizon": int(horizon),
        "step_m": float(step_m),
    }
