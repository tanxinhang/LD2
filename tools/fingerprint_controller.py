"""Write the deterministic complete-controller/config fingerprint."""
...
def fingerprint_document(config_path: Path) -> dict[str, object]:
    result = fingerprint_controller_implementation(
        config_path, workspace_root=ROOT)
    return {
        "schema_version": result.schema_version,
        "sha256": result.sha256,
        "scope": (
            "all_uav_isac_python_plus_audit_and_energy_calibrator_plus_"
            "complete_config_chain"),
