#!/usr/bin/env python
"""M4-A/B task-equivalent dense-vs-factor rate audit."""

from __future__ import annotations

import argparse
from collections import Counter
import json
from pathlib import Path
import sys

import numpy as np

ROOT = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(ROOT))

from config.params import load_config  # noqa: E402
from uav_isac.coordination.certified_factor_message import (  # noqa: E402
    IntervalCode, certify_fixed_plan_three_floors,
    factor_coefficient_envelope, linear_interval_quantize_positive,
    log_interval_quantize, task_equivalent_payload_bits,
)
from uav_isac.coordination.coefficient_structure import (  # noqa: E402
    BistaticFactorization, balance_bistatic_factor_gauge,
    exact_bistatic_factorization,
)
from uav_isac.coordination.minimum_intervention_repair import (  # noqa: E402
    solve_minimum_intervention_task_repair_milp,
)
from uav_isac.coordination.scale_capability import cap_aware_sensing_budget  # noqa: E402
from uav_isac.evaluation.physical_oracle_audit import (  # noqa: E402
    per_watt_deflection_tensor_from_observables,
)
from uav_isac.physical.detection import compute_detection_probabilities  # noqa: E402


METHODS = ("dense_linear", "dense_log", "factor_log")
BITS = (3, 4, 6, 8, 10, 12, 14, 16)
HEADROOMS = (0.0025, 0.005, 0.01)


def _true_feasible(coefficient, owner, power, p_fa, floors) -> bool:
    Q = coefficient.shape[2]
    d = np.asarray([
        np.dot(coefficient[:, owner[q], q], power[:, q]) for q in range(Q)])
    pd = compute_detection_probabilities(d, p_fa)
    tail_k = int(floors[3])
    metrics = (float(np.min(pd)),
               float(np.mean(np.partition(pd, tail_k - 1)[:tail_k])),
               float(np.mean(pd)))
    return all(value >= float(floor) - 1.0e-10
               for value, floor in zip(metrics, floors[:3]))


def audit(trace_path: Path, config_path: Path, g4a_path: Path) -> dict[str, object]:
    with np.load(trace_path, allow_pickle=False) as loaded:
        data = {key: loaded[key] for key in loaded.files}
    cfg = load_config(str(config_path))
    g4a = json.loads(g4a_path.read_text(encoding="utf-8"))
    region = {(int(row["seed"]), int(row["frame"])) for row in g4a["rows"]}
    floors = tuple(cfg.marl.task_constrained_qos_floors)
    rows = []
    containment = {method: {str(bits): 0 for bits in BITS} for method in METHODS}
    for index in range(np.asarray(data["seed"]).size):
        key = (int(data["seed"][index]), int(data["frame"][index]))
        if key not in region:
            continue
        alpha = np.asarray(data["privileged_alpha"][index], dtype=np.float64)
        report = np.asarray(data["privileged_chi_rep"][index], dtype=np.float64)
        dd = np.asarray(data["privileged_g_dd"][index], dtype=np.float64)
        active = dd >= float(cfg.detection.g_min)
        coefficient = per_watt_deflection_tensor_from_observables(
            alpha, dd, report,
            T_sym=float(cfg.otfs.T_sym), M=int(cfg.otfs.M), N=int(cfg.otfs.N),
            kT=float(cfg.channel.kT), bandwidth_hz=float(cfg.otfs.B),
            noise_figure_db=float(cfg.channel.NF),
            g_tx_dbi=float(cfg.otfs.g_tx_dBi), g_rx_dbi=float(cfg.otfs.g_rx_dBi),
            n_cpi=int(cfg.otfs.n_cpi), g_min=float(cfg.detection.g_min),
            use_swerling=bool(cfg.channel.use_swerling))
        K, _, Q = coefficient.shape
        support = active & ~np.eye(K, dtype=bool)[:, :, None]
        exceptions = int(Q * K * (K - 1) - np.sum(support))
        dense_values = coefficient[support]
        factors = []
        for q in range(Q):
            raw = exact_bistatic_factorization(
                alpha[:, :, q] ** 2, report[:, :, q], active[:, :, q])
            raw_dense = raw.dense()
            positive = raw_dense > 0.0
            detector_scale = float(np.median(
                coefficient[:, :, q][positive] / raw_dense[positive]))
            factors.append(balance_bistatic_factor_gauge(BistaticFactorization(
                raw.tx_factor, raw.rx_factor * detector_scale,
                raw.inactive_offdiagonal)))
        factor_values = np.concatenate([
            np.concatenate([factor.tx_factor, factor.rx_factor])
            for factor in factors])

        envelopes = {method: {} for method in METHODS}
        for bits in BITS:
            for method in ("dense_linear", "dense_log"):
                code = (linear_interval_quantize_positive(dense_values, bits)
                        if method == "dense_linear"
                        else log_interval_quantize(dense_values, bits))
                lower = np.zeros_like(coefficient)
                upper = np.zeros_like(coefficient)
                lower[support] = code.lower
                upper[support] = code.upper
                envelopes[method][bits] = (lower, upper)
            code = log_interval_quantize(factor_values, bits)
            lower = np.zeros_like(coefficient)
            upper = np.zeros_like(coefficient)
            offset = 0
            for q in range(Q):
                tx = IntervalCode(code.lower[offset:offset + K],
                                  code.upper[offset:offset + K],
                                  code.scale_lower, code.scale_upper, bits)
                rx = IntervalCode(code.lower[offset + K:offset + 2 * K],
                                  code.upper[offset + K:offset + 2 * K],
                                  code.scale_lower, code.scale_upper, bits)
                offset += 2 * K
                lower[:, :, q], upper[:, :, q] = factor_coefficient_envelope(
                    tx, rx, active[:, :, q])
            envelopes["factor_log"][bits] = (lower, upper)
            for method in METHODS:
                low, high = envelopes[method][bits]
                containment[method][str(bits)] += int(np.sum(
                    (coefficient < low * (1.0 - 1.0e-12))
                    | (coefficient > high * (1.0 + 1.0e-12))))

        rate = np.asarray(data["outgoing_rate"][index], dtype=np.int64)
        token_mask = np.asarray(data["outgoing_token_mask"][index], dtype=bool)
        comm_active = (rate > 0) & np.any(token_mask, axis=1)
        comm = np.where(comm_active, np.clip(np.asarray(
            data["comm_fraction"][index]), 0, 1) * float(cfg.uav.P_isac_total), 0)
        budget = cap_aware_sensing_budget(
            float(cfg.uav.P_isac_total), comm, float(cfg.uav.P_sense_max))
        role = np.asarray(data["teacher_role"][index], dtype=np.int8)
        selected0 = np.asarray(data["teacher_pair"][index], dtype=bool)
        owner0 = np.asarray(data["teacher_receiver_owner"][index], dtype=np.int64)
        common = dict(
            p_fa=float(cfg.detection.P_FA),
            target_pair_limit=int(np.asarray(data["target_pair_limit"]).reshape(-1)[0]),
            reports_per_receiver=int(np.asarray(data["reports_per_receiver"]).reshape(-1)[0]),
            pwl_epsilon=1.0e-3, time_limit_s=30.0)
        for headroom in HEADROOMS:
            tightened = (float(floors[0]) + headroom,
                         float(floors[1]) + headroom,
                         float(floors[2]) + headroom, int(floors[3]))
            witness = solve_minimum_intervention_task_repair_milp(
                coefficient, budget, selected0, role == 0, role == 1, owner0,
                task_floors=tightened, **common)
            if not witness.feasible:
                raise RuntimeError(f"headroom witness failed for {key}, {headroom}")
            truth = _true_feasible(
                coefficient, witness.receiver_owner, witness.sensing_power_w,
                float(cfg.detection.P_FA), floors)
            for method in METHODS:
                for bits in BITS:
                    lower, upper = envelopes[method][bits]
                    certificate = certify_fixed_plan_three_floors(
                        lower, upper, witness.receiver_owner,
                        witness.sensing_power_w, p_fa=float(cfg.detection.P_FA),
                        task_floors=floors)
                    payload = task_equivalent_payload_bits(
                        method, K, Q, bits, exceptions).total_bits
                    rows.append({
                        "seed": key[0], "frame": key[1],
                        "headroom": headroom, "method": method, "bits": bits,
                        "payload_bits": payload, "status": certificate.status,
                        "true_feasible": truth,
                        "false_cert_feasible": (
                            certificate.status == "CERT_FEASIBLE" and not truth),
                    })

    summary = {}
    adaptive = {}
    for headroom in HEADROOMS:
        summary[str(headroom)] = {}
        adaptive[str(headroom)] = {}
        for method in METHODS:
            method_rows = [row for row in rows
                           if row["headroom"] == headroom and row["method"] == method]
            by_bits = {}
            for bits in BITS:
                selected = [row for row in method_rows if row["bits"] == bits]
                by_bits[str(bits)] = {
                    "cert_feasible": sum(
                        row["status"] == "CERT_FEASIBLE" for row in selected),
                    "false_cert_feasible": sum(
                        row["false_cert_feasible"] for row in selected),
                }
            summary[str(headroom)][method] = by_bits
            choices = []
            for key in sorted({(row["seed"], row["frame"]) for row in method_rows}):
                candidates = sorted(
                    (row for row in method_rows
                     if (row["seed"], row["frame"]) == key
                     and row["status"] == "CERT_FEASIBLE"),
                    key=lambda row: (row["payload_bits"], row["bits"]))
                choices.append(candidates[0] if candidates else None)
            certified = [choice for choice in choices if choice is not None]
            adaptive[str(headroom)][method] = {
                "certified_frames": len(certified),
                "min_bit_distribution": dict(sorted(Counter(
                    choice["bits"] for choice in certified).items())),
                "payload_min": min(choice["payload_bits"] for choice in certified)
                if certified else None,
                "payload_median": float(np.median([
                    choice["payload_bits"] for choice in certified]))
                if certified else None,
                "payload_max": max(choice["payload_bits"] for choice in certified)
                if certified else None,
            }
    comparisons = {}
    for headroom in HEADROOMS:
        factor = adaptive[str(headroom)]["factor_log"]
        dense_candidates = [adaptive[str(headroom)][name]
                            for name in ("dense_linear", "dense_log")]
        best_dense = min(
            (item for item in dense_candidates if item["certified_frames"] == len(region)),
            key=lambda item: item["payload_median"], default=None)
        comparisons[str(headroom)] = {
            "factor_all_certified": factor["certified_frames"] == len(region),
            "factor_median_bits": factor["payload_median"],
            "best_dense_median_bits": (
                best_dense["payload_median"] if best_dense is not None else None),
            "factor_over_best_dense": (
                factor["payload_median"] / best_dense["payload_median"]
                if best_dense is not None and factor["certified_frames"] == len(region)
                else None),
        }
    false_certs = sum(row["false_cert_feasible"] for row in rows)
    return {
        "gate": "M4-A/B-task-equivalent-rate",
        "scope": "21 correlated development frames; semantic shadow, centralized dynamic scale",
        "methods": list(METHODS), "bits": list(BITS),
        "headrooms": list(HEADROOMS),
        "fairness": (
            "common 27-bit header and exact DD codec; dense methods code only active entries; "
            "all methods use adaptive first-certified payload"),
        "containment_violations": containment,
        "false_cert_feasible": false_certs,
        "summary": summary, "adaptive": adaptive,
        "comparisons": comparisons,
        "safety_gate_pass": false_certs == 0 and all(
            value == 0 for method in containment.values() for value in method.values()),
        "communication_value_gate_pass": any(
            item["factor_over_best_dense"] is not None
            and item["factor_over_best_dense"] < 1.0
            for item in comparisons.values()),
        "rows": rows,
    }


def main() -> None:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--trace", required=True, type=Path)
    parser.add_argument("--config", required=True, type=Path)
    parser.add_argument("--g4a", required=True, type=Path)
    parser.add_argument("--output", required=True, type=Path)
    args = parser.parse_args()
    result = audit(args.trace, args.config, args.g4a)
    payload = json.dumps(result, indent=2, sort_keys=True)
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(payload + "\n", encoding="utf-8")
    print(json.dumps({key: value for key, value in result.items()
                      if key not in {"rows", "summary"}},
                     indent=2, sort_keys=True))


if __name__ == "__main__":
    main()
