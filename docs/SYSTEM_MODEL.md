...
> **版本说明（2026-08-01）：**本文主要记录基础环境和历史 P0 数学模型。
> 当前部署边界已演进为“分布式运动/通信/结构 Student 决策 + 环境级集中式证据融合
> 检测”；冻结集中式 P0 只作为教师和参考控制。当前架构、结果与可宣称范围以
> [`CURRENT_SYSTEM_STATUS.md`](CURRENT_SYSTEM_STATUS.md) 为准，本文中的“默认 P0 承担”
> 不应解释为当前部署策略仍直接调用集中式教师。
