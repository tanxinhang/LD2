...
### Stage-11 preregistered feasibility-first confirmation (2026-09-09)

Before confirmation, a non-saturated `0.15 W` per-UAV sensing-cap regime was
selected using only training seed `31415` and evaluation seeds
`32001--32010`. The frozen protocol was committed as `fa6da02` before any
confirmation run. It reserved new training seeds `41001--41005`, new paired
evaluation seeds `33001--33020`, worst-target detection as the primary
endpoint, and fixed effect, replication, QoS, CVaR-area, RF, KL and rollback
gates. The candidate differs from control only by enabling scale-invariant
feasibility-first restoration while the temporal CVaR residual is above
tolerance.

The primary result is negative. Across the five independent training seeds,
candidate-minus-control worst-target `P_D` is `-0.01749` on average (median
`-0.00777`, cluster-bootstrap 95% CI `[-0.15113, +0.11364]`). Two seeds
improve and three degrade; only two exceed the preregistered `+0.001`
replication threshold. Mean evaluation QoS feasibility decreases from `0.79`
to `0.78`.

The mechanism is active on all 15 candidate updates, but the summed positive
CVaR residual is identically `1.797` in both arms. Thus the restoration signal
is saturated rather than progressively informative in this low-cap regime.
Hard RF feasibility remains intact, but seed `41003` exceeds the attempted-KL
gate and triggers two transactional actor rollbacks. The candidate fails the
primary effect, confidence, replication, QoS and safety conditions in the
precommitted decision rule.

This hypothesis is therefore falsified under its preregistered protocol and
is not promoted. The implementation remains default-off solely so the result
and source snapshots are reproducible; it is retired from the active
optimization path. The compact hash-bound evidence is
`artifacts/research/feasibility_first_confirmation_v1.json`, generated by
`tools/summarize_feasibility_first_confirmation.py`.

The scientific implication is stronger than “try a smaller step”: the current
calibrated temporal CVaR surrogate can saturate at one nearly undetected target
and then provides no graded measure of recovery. A future candidate must first
replace that saturated maximum-tail signal (for example with a smooth
target-wise shortfall distribution or continuation floor), and it must show
surrogate-to-executed monotonicity on a calibration bank before consuming a
