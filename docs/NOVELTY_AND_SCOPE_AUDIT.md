...
> 审计日期：2026-07-29（**2026-08-16 数字修正**：头部"正式部署结果"更新为当前
> 100 种子复核值；原 0.890/0.854/0.679 为旧部署版，见正文 §5.3 历史对照）。
> 审计性质：面向论文立项的机制级快速审计，不等同于穷尽式系统综述或专利查新。  
> 当前正式部署结果（100 种子复核，`assert_formal_gates.py`）：
> `steady=0.9132`、`weak3=0.8848`、`worst=0.7393`、
> `CVaR=0.2768`、QoS 可行率 `0.72`，且严格满足单 UAV `1 W` 通信–感知总功率约束。
...
冻结结构教师后又完成了更严格的 2 种子分布式 Gate B2。所有版本均只使用实际
到达的公共量化 Token，并加入单接收机所有者、公共状态和 5 帧滞回：

| 2-seed protocol | steady | weak3 | worst | target coverage | bit/frame |
|---|---:|---:|---:|---:|---:|
| 同种子冻结基线 | 0.756 | 0.675 | **0.398** | -- | 768 |
| 4-bit 绝对位置重构 | 0.751 | 0.668 | 0.070 | 0.979 | 1728 |
| 指数端点能力代理 | 0.798 | 0.731 | 0.341 | 0.958 | 1467 |
| 逆平方因式化能力 | **0.832** | **0.776** | 0.344 | **0.991** | 1472 |

绝对位置在 800 m 区域内以 4 bit 量化时分辨率约 53 m，会颠倒边排序。因式化
能力避免了绝对位置并恢复 steady/weak3，但两个 episode 的 worst 为
`0.094/0.594`，仍会饿死单个瓶颈目标。由此可排除“覆盖不足”和“缺少滞回”
作为剩余主因；下一缺失变量是所有节点共同认可的**目标缺口价格**。Gate B2
未扩到 10 种子，后续不得继续调距离尺度或 Token 维度。
