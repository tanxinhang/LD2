...
绕过 provenance。
## P0 active-geometry survival gate (implementation scaffold)

`physical/active_information.py` now provides the deliberately small interface
requested for the next research line: finite motion candidates, physical
feasibility, half-duplex single-Tx roles, a fail-closed Schur-complement
conditional-information calculation, and Top-M exact max-min validation.
`tools/audit_active_geometry_survival.py` is a K3/Q1 frozen-waveform,
frozen-sensing-power G0. It uses actual bistatic radar amplitudes and fractional
OTFS DD templates, while an explicitly declared cross-frame Gaussian clutter
correlation supplies the first conditional-information falsification case.

This gate must not be reported as general active-ISAC superiority. It freezes
the Tx role, uses coherent Gaussian detection, and assumes rather than learns
the temporal clutter covariance. Its purpose is to test whether the proposed
control chain can create measurable information without power confounding.
Communication power is exactly zero because G0 sends no message.

With 200,000 held-out trials at design P_FA=0.001, fixed geometry/role gives
theoretical/empirical P_D 0.34957/0.35024. Motion with the role frozen gives
0.36573/0.36624. Opening the finite single-Tx role set and motion raises this to
0.63992/0.64091 at the same 0.15 W sensing power. The asymmetric near-800 m
geometry is necessary: a separate equidistant, static counterexample confirms
that reciprocal bistatic DD paths make role changes exactly uninformative.

History-conditioned and instantaneous-information scoring still select the
same action and have identical theoretical P_D. Their empirical difference is
independent Monte Carlo jitter and is not counted as an algorithmic gain. Thus
P0 now supports active geometry plus role selection, still falls below the
required P_D>0.8, and **does not support a history-novelty claim**.

## G0.5 complete short-history closure

The next audit retains every locally available observation rather than only
the previous epoch. Receiver-local temporal covariance is generated by an
AR(1) time kernel multiplied by the actual OTFS-template Gram kernel; covariance
is zero across distinct receiver-local clutter processes. This construction is
PSD by the Schur product theorem and is checked numerically. At every epoch the
instantaneous baseline maximizes fresh information, whereas the proposed policy
maximizes the Schur-complement increment conditioned on its complete history.

At horizons 1/2/3/4/5 the history policy obtains theoretical P_D
0.3853/0.6645/0.7343/0.7749/0.8266 at P_FA=0.001. The instantaneous policy gives
0.3853/0.4334/0.4872/0.5459/0.6079. Therefore the declared P_D>0.8 gate is first
crossed after five 0.8 s slow-geometry epochs (4.0 s), not after five OTFS
frames. At that point both policies expend the same 1085.0 J modelled flight
energy, while sensing RF energy is only 0.000768 J. This sharp difference is why
propulsion and RF power cannot share one budget.

This is meaningful but still conditional evidence: the target is static, the
Gaussian temporal covariance is known, and the evidence is local, so U2U
communication is neither needed nor charged. Moving-target covariance learning
and an actual no-ACK message path remain later gates.

## G0.6 moving-target history alignment

A deterministic CV target at [12,6,0] m/s is now propagated on the same 0.8 s
slow clock. Candidate DD signatures use the predicted target state, while the
stored observation is rebuilt at the true state for offline evaluation. At five epochs, matched CV
prediction yields history-conditioned P_D 0.9934 versus 0.6926 for the
instantaneous-action baseline under the same moving-target diversity. A stale
zero-velocity prediction reaches 0.9315 after the information-boundary correction,
so target motion itself supplies substantial DD diversity; matched-minus-stale
0.0619 is the alignment gain
and must not be added to the within-case policy gain.

This result preserves the P_D>0.8 gate under one severe stale-belief case, but
is not yet robust tracking evidence: CV truth is deterministic, belief/process
noise is absent, and covariance is known. The next valid falsification step is
a seeded maneuver/measurement-error ensemble with a paired lower confidence
bound, not another favourable deterministic trajectory.

An information-boundary correction changed the earlier opposite-velocity
result. The controller had incorrectly retained truth-rebuilt templates as its
next predicted history, leaking oracle state into action selection. Predicted
history and truth-only evaluation history are now separate. After correction,
stale and opposite predictions obtain P_D 0.9315 and 0.9918 respectively,
versus 0.9934 for matched CV. The previous claimed opposite-bias failure is
withdrawn. The corrected result instead shows that role diversity dominates
this deterministic case, so it cannot expose velocity robustness. A generic
unweighted max-min selector over explicit state hypotheses is added, but its
benefit must be established on a non-degenerate uncertainty ensemble.
