...
- 处理：盲测结论如实披露为"点估计 0.730 + LCB 0.636"；论文 `--require-lcb`
  声明不成立，须改用点估计 + 置信区间，或扩大样本/方差缩减（分层抽样、
  公共随机数配对）后重新认证。
- 附加发现（左尾分解）：失败 seed 为场景级塌陷，主导因子是初始最差目标-最近
  UAV 距离 `worst_nearest`（<350 m QoS 0.91 → >550 m 0.17）；episode 最大位移
  375 m（v_max 25 m/s × 15 s），realized 距离与 worst P_D 相关 −0.863，
  **>450 m 部分物理不可达**，300–450 m 为策略可改进区（D1.9 候选）。
- 优先级：P1（不阻塞论文主结果，但决定声明口径与下一步优化重点）。
