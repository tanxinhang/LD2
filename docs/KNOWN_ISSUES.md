...
**D1.5 盲测统计功效（2026-08-16 新增，D1.9 已闭合）**：
- 现象：frozen 候选在 100 全新 blind seed 上 **QoS feasible 0.730（点估计过 0.70）
  但 Wilson LCB 0.636 < 0.70**（95% 单侧，z=1.96）；与 4/4 观察（LCB 0.625）
  同构——**N=100 的统计功效不足以 95% 置信保证 QoS ≥ 0.70**。
- **D1.9 闭合（2026-08-16）**：瓶颈前瞻 L3（`analytical_movement_lookahead_frames
  =40`，receding-horizon 评分）修复单步 trust-region 停滞，同 100 blind seed 上
  QoS **0.950**、LCB **0.888**（≥0.70）——`--require-lcb` 论文声明成立。
  剩余失败 4/5 为 worst_nearest >450 m 的物理不可达 seed（episode 375 m 位移
  预算边界，见 OPTIMIZATION_LOG D1.9）；残余策略失败仅 seed 615（355 m，
  QoS 0.43）。若论文需 100% 达标，须扩展 episode 时长或 v_max（改变运动学
  设定，属场景定义变更）。
- 优先级：已闭合（P1 → 文档记录）。
